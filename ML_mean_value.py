...
class RadiomicsClassifier:
    ...
    def load_data(self,feature_type):
        ...
        if feature_type == 'ADC':
            feature_cols = [col for col in self.data.columns if col not in ['Patient_ID', 'label', 'fit_d','fit_vin','fit_Din','fit_Dex']]
        if feature_type == 'micro':
            feature_cols = [col for col in self.data.columns if col not in ['Patient_ID', 'label', 'ADC_N_1','ADC_N_2','ADC_PGSE']]
        if feature_type == 'all':
            feature_cols = [col for col in self.data.columns if col not in ['Patient_ID', 'label']]
    ...
    def lasso_feature(self,feature_type):
        scaler = StandardScaler()
        X_reduced = scaler.fit_transform(self.X_reduced)
        pca = PCA(n_components=0.9, random_state=42)
        X_lasso = pca.fit_transform(X_reduced)
        ...
        if hasattr(self, 'feature_names') and self.feature_names is not None:
            ...
            print("warning: failed to get feature names, use default numbering")
            feature_names = [f'X{i+1}' for i in range(pca.components_.shape[1])]
        ...
        for i in range(n_components):
            ...
            for j, weight in enumerate(component_weights):
                if abs(weight) > 0.001: 
                    feature_name = feature_names[j] if j < len(feature_names) else f'X{j+1}'
                    expression_parts.append(f"{weight:.2f}*{feature_name}")
            
            if expression_parts:
                expression = " + ".join(expression_parts)
                print(f"PC{i+1} (explained variance ratio: {variance_ratio:.4f}):")
                print(f"  {expression}")
        
        components_df = pd.DataFrame(pca.components_.T, 
                                    index=feature_names,
                                    columns=[f'PC{i+1}' for i in range(n_components)])
        components_df.to_csv(f'results/mean_{feature_type}_pca_components.csv', encoding='utf-8-sig')
        print(f"\nsave to: results/mean_{feature_type}_pca_components.csv")
    ...
    def cross_validation(self,feature_type):
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X_cv, y_cv)):
            ...
            for name, base_clf in classifiers.items():
                if feature_type == 'ADC':
                    if name == 'SVM':
                        fold_clf = SVC(probability=True, random_state=42,C=3.0,gamma=1.5,kernel='poly')
                        ...
                        fold_clf = RandomForestClassifier(random_state=42,max_depth=4,
                                                        min_samples_split=7,n_estimators=200)
                    elif name == 'GradientBoosting':
                        fold_clf = GradientBoostingClassifier(learning_rate=0.1,max_depth=5,n_estimators=50,random_state=42)
                    elif name == 'KNN':
                        fold_clf = KNeighborsClassifier(metric='euclidean',n_neighbors=11,weights='distance')
                
                elif feature_type == 'micro':
                    if name == 'SVM':
                        fold_clf = SVC(probability=True, random_state=42,C=2.0,gamma=2,kernel='rbf')
                    elif name == 'GaussianNB':
                        fold_clf = GaussianNB(var_smoothing=0.013)
                    elif name == 'LR':
                        fold_clf = LogisticRegression(C=10,solver='lbfgs', random_state=42, max_iter=1000)#
                    elif name == 'RandomForest':
                        fold_clf = RandomForestClassifier(random_state=42,max_depth=5,
                                                        min_samples_split=7,n_estimators=100)
                    elif name == 'GradientBoosting':
                        fold_clf = GradientBoostingClassifier(learning_rate=0.01,max_depth=3,n_estimators=50,random_state=42)
                    elif name == 'KNN':
                        fold_clf = KNeighborsClassifier(metric='euclidean',n_neighbors=7,weights='distance')
                    
                elif feature_type == 'all':
                    if name == 'SVM':
                        fold_clf = SVC(probability=True, random_state=42,C=4.0,gamma='scale',kernel='linear')
                    elif name == 'GaussianNB':
                        fold_clf = GaussianNB(var_smoothing=0.013)
                    elif name == 'LR':
                        fold_clf = LogisticRegression(C=1,solver='liblinear',random_state=42, max_iter=1000)
                    elif name == 'RandomForest':
                        fold_clf = RandomForestClassifier(random_state=42,max_depth=7,
                                                        min_samples_split=10,n_estimators=200)
                    elif name == 'GradientBoosting':
                        fold_clf = GradientBoostingClassifier(learning_rate=0.1,max_depth=2,n_estimators=50,random_state=42)
                    elif name == 'KNN':
                        fold_clf = KNeighborsClassifier(metric='minkowski',n_neighbors=13,weights='uniform')
                ...
                if len(np.unique(y_fold_test)) > 1:
                    fpr, tpr, thresholds = roc_curve(y_fold_test, y_pred_proba)
                    auc_score = roc_auc_score(y_fold_test, y_pred_proba)
                    cv_results[name]['roc_curves'].append({
                        'fpr': fpr,
                        'tpr': tpr,
    ...
    def run_complete_pipeline(self, feature_type='default'):
        self.load_data(feature_type)
        
        self.normalize_data()
        
        # 3. LASSO feature selection only（feature_type=all）
        # self.lasso_feature(feature_type)
